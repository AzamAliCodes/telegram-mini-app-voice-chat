    ...
    <div className="absolute inset-0 flex flex-col bg-[#3A2570]/95 backdrop-blur-xl z-10 rounded-[24px]">
      ...
      <div ref={listRef} className="flex-1 overflow-y-auto px-4 py-3 space-y-2">
        ...
        {messages.map((msg) => {
          ...
          return (
            <div key={msg.id} className={`flex gap-2.5 mb-4 ${isMine ? 'flex-row-reverse' : 'flex-row'}`}>
              {/* Avatar */}
              <div className="flex-shrink-0 mt-auto">
                {msg.photo_url ? (
                  <img src={msg.photo_url} alt={msg.sender_name} className="w-8 h-8 rounded-full object-cover border border-white/10" />
                ) : (
                  <div className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center text-[10px] font-bold text-white/50 border border-white/10">
              ...
              <div className={`flex flex-col max-w-[75%] ${isMine ? 'items-end' : 'items-start'}`}>
                {!isMine && (
                  <span className="text-white/40 text-[10px] ml-1 mb-1">{msg.sender_name}</span>
                )}
                <div className={`px-3.5 py-2.5 rounded-[18px] text-sm leading-relaxed break-words shadow-sm ${
                  isMine
                    ? 'bg-[#5B6BC0] text-white rounded-tr-[4px]'
                    : 'bg-white/15 text-white/90 rounded-tl-[4px]'
                }`}>
                  {msg.text}
                </div>
              </div>
